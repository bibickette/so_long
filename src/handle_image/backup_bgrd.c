...
void	cpy_background(t_long *so_long)
{
	...
	while (++y < (so_long->map_size * IMG_HEIGHT))
	{
		...
		{
			color = *(unsigned int *)(so_long->background.addr + ((y * so_long->background.line_len
...
void	put_backup(t_long *so_long, int where_x, int where_y)
{
	...
	while (++y < IMG_HEIGHT)
	{
		...
		while (++x < IMG_WIDTH)
		{
			color = *(unsigned int *)(so_long->backup.addr + (((y + where_y * IMG_HEIGHT) * so_long->backup.line_len 
					+ (x + where_x * IMG_WIDTH) * ((so_long->backup.bpp / 8)))));
